 ...
 * 获取指定用户的QQ号码
 * 从uin --> qq 的映射
 * {"retcode":0,"result":{"uiuin":"","account":6208317,"uin":4231694705}}
 **/
Members.prototype.fetchBuddyQQAccount = function(uin, defer) {

	var account = this.account;

	var url = util.format("http://s.web2.qq.com/api/get_friend_uin2?tuin=%s&verifysession=&type=1&code=&vfwebqq=%s&t=" + new Date().getTime(),
		uin,
	...
	then(function(d) {
		...
		if (body && body.retcode == 0) {
			//更新idmap中的uin -> qq映射
			var idmap = account.idmap || {};
			//更新mapid中的qq -> uin映射
			var mapid = account.mapid || {};
			idmap[uin] = body.result.account;
			mapid[body.result.account] = uin;
