...
function Session(account, httpclient) {
	...
	fetchEQQAll(function(map){
		faceMap = map;
	})
}


function fetchEQQAll(callback) {
	this.httpclient.get("http://0.web.qstatic.com/webqqpic/pubapps/0/50/eqq.all.js?t=20130903001", function(error, res, body) {
		var s = body.indexOf('T_TRANSFER_TABLE');
		var t = body.indexOf('WEBQQMSGTIPS');
		var c = "var map =" + body.substring(s + 'T_TRANSFER_TABLE'.length + 1, t - 1);
		eval(c);
		callback(map);
	})
}
