				...
				for (var i = results.length - 1; i >= 0; i--) {
					...
					if (pollType === "input_notify") {
						...
						//notifyEvents.add(processGroupMsg(pollData));
						 processGroupMsg(pollData, account, qunService, defer);
						...
						//notifyEvents.add(processSessionMsg(pollData));
						 processSessMsg(pollData, account, defer);
...
function processGroupMsg(pollData, account, qunService, defer) {
...
Session.prototype.downloadGroupImage = function(file, gid) {
	var httpclient = this.httpclient;
	var account = this.account;
	...
	httpclient.get("http://web.qq.com/cgi-bin/get_group_pic?gid=" + gid + "&uin=" + account.clientid + "&fid=" + file.file_id + "&pic=" + file.name + "&vfwebqq=" + account.session.vfwebqq).pipe(stream);
}
...
function processSessMsg(pollData, account, defer) {
