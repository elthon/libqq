...
*** gnumber 群号码
***/
QQClient.prototype.searchQun = function(gnumber,code, page){
	var self = this;
	then(function(defer){
		self.qunModule.searchQun(defer, gnumber, code, page, 10);
	}).then(function(defer, result){
		self.emit("searchQunResult", result);
	});
}
...
QQClient.prototype.downloadGroupImage = function(msg) {
	var sessionModule = this.sessionModule;
	_.each(msg.pics, function(pic) {
		sessionModule.downloadGroupImage(pic, msg.from.uin,  msg.group_code);	//发送者的临时id， 群临时id
	})
}
