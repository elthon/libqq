...
function saveFile(body, url) {
	try {
		var t = JSON.stringify(body, url);
		fs.appendFile('data/message.json', url + " ===> " + t + "\r\n", function(err) {
			if (err)
				console.log("保存message.json出错。", err);
		});
	} catch (e) {
		fs.appendFile('data/message.json', url + " ===> " + body + "\r\n", function(err) {
			if (err)
				console.log("保存message.json出错。", err);
		});
	}
